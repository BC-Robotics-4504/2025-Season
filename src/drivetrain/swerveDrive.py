...
class SwerveDrive:
    ...
    def __init__(self, config: SwerveConfig) -> None:
        ...
        self.fl_mod = SwerveModule(
            config.fl_CAN, config.fl_zoffset, config.wheel_diameter
        )
        self.fr_mod = SwerveModule(
            config.fr_CAN, config.fr_zoffset, config.wheel_diameter
        )
        self.rl_mod = SwerveModule(
            config.rl_CAN, config.rl_zoffset, config.wheel_diameter
        )
        self.rr_mod = SwerveModule(
            config.rr_CAN, config.rr_zoffset, config.wheel_diameter
        )
    ...
    def move(self, Lx: float, Ly: float, Rx: float):
        ...
        Vx0 = (
            -Lx
            * self.config.max_driving_speed
            * self.config.drive_wheel_diameter
            * math.pi
        )
        Vy0 = (
            Ly
        ...
        self.__frontLeftSpeed__ = math.hypot(Vyp, Vxp) / (
            math.pi * self.config.drive_wheel_diameter
        )
    ...
    def goDistance(self, Rx0: float, Ry0: float, r0: float):
        """SwerveDrive.goDistance(target_distance: float, target_angle: float, target_rotations: float)
    ...
    def updateDrive(self):
        """
        Updates the Swerve Modules with their new speeds and angles
        """
